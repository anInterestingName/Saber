# GitHub Actions 构建并发布私有 GHCR 镜像测试文档
...
### TC-004 真实 GHCR 私有发布与拉取

- 优先级：P0
- 关联验收标准：`AC-001`、`AC-002`、`AC-003`
- 模块与入口：GitHub Actions、GitHub Packages、目标服务器。
- 前置条件：配置提交并推送；仓库允许 Actions 写 Packages；准备安全保存的只读拉取 Token。
- 依赖：`TC-001` 至 `TC-003`。
- 可并行：否；依赖真实远端发布结果。
...
- 测试步骤：
  ...
  3. 在 Packages 页面确认包为 Private 并已关联当前仓库。
  4. 在目标服务器使用仅有 `read:packages` 的 Token 登录并拉取镜像。
  5. 启动容器并执行根路径、子路由和 `/healthz` 冒烟检查。
- 预期结果：私有镜像发布成功；未认证无法拉取；只读 Token 可以拉取并正常运行。
- 证据要求：Actions 运行链接、包设置截图、镜像 digest 和脱敏后的拉取结果。
- 清理要求：保留 `manual` 包供验收；停止测试容器，不删除正式或历史版本。
- 实际结果：配置尚未提交并推送，GitHub Actions 无法读取当前工作区文件，因此未执行真实 GHCR
  发布、Private 可见性和目标服务器只读拉取。
- 结果：阻塞
- 缺陷引用：`ENV-001`
...
| --- | --- | --- | --- | --- |
| ENV-001 | TC-004 | 阻塞 | 配置尚未提交并推送，无法执行真实 GitHub Actions、GHCR 私有拉取和 Compose 启动验收 | 待远端执行 |
...
- 未完成项：真实 GHCR 私有发布、Packages Private 可见性、目标服务器只读拉取和 Compose 启动。
