...
| 创建日期 | 2026-09-09 |
| 最后更新日期 | 2026-09-16 |
| 关联事项 | [需求索引](../requirements-index.md)；[详细设计 0.5](../design/DESIGN-REQ-2026-009-prompt-management.md)；[测试文档 0.5](../test/TEST-REQ-2026-009-prompt-management.md)；数据库设计不涉及 |
| 后端基线 | SpringBlade REQ-2026-001 0.3、REQ-2026-005 0.5 及当前 blade-ai 实现 |
...
7. 支持提示词类型和发布方式筛选、编辑与展示，并在自动发布保存前明确立即上线后果。
...
- 按提示词类型和发布方式筛选，列表、详情和版本展示稳定类型及来源名称。
- 编辑器支持普通发布与自动发布；自动发布要求发布权限和变更说明，保存成功后直接返回新版本。
...
- 查看详情时展示当前草稿、服务端警告、当前发布版本摘要和可执行动作状态；存在已发布版本时只读展示该版本的变更说明，不显示新增说明校验。
...
- `AC-019`：Given 提示词存在当前发布版本或用户选择任一历史版本，When 从主列表或版本抽屉查看详情，Then 当前或选中版本的变更说明和完整快照只读展示，正文和变量值按纯文本处理，页面不要求填写新的变更说明。
...
- `AC-030`：Given 1440px、1024px 和 375px 视口，When 使用查询、列表、编辑、预览和版本功能，Then 列表各业务列按最小宽度共同分配可用空间，名称和编码不独占剩余宽度，文字与控件不重叠且主要按钮可操作。
...
| 2026-09-16 | 0.4 | 对齐 SpringBlade REQ-2026-005，增加提示词类型、发布方式、自动发布保存和版本来源展示 | 后端契约新增类型、MANUAL/AUTO 与 DataScope 行为 | 提示词 API、列表、编辑器、预览、版本抽屉和测试计划 | Codex |
| 2026-09-16 | 0.5 | 调整列表业务列为共同弹性分配宽度；查看模式回填当前发布版本变更说明并取消只读必填提示 | 用户反馈宽屏列表列宽失衡，且查看模式错误提示填写不可编辑的变更说明 | 提示词列表、查看弹窗、详细设计和测试用例 | Codex |
