...
| 创建日期 | 2026-09-14 |
| 最后更新日期 | 2026-09-16 |
| 关联事项 | [需求索引](../requirements-index.md)；[详细设计 0.5](../design/DESIGN-REQ-2026-012-ui-normalization-spacious-visual-refresh.md)；[测试文档 0.3](../test/TEST-REQ-2026-012-ui-normalization-spacious-visual-refresh.md)；数据库设计不涉及；承接 [REQ-2026-011](REQ-2026-011-pro-style-ui-record-panels.md) 已建立的主题 Token、导航、页容器和记录弹层能力 |
...
并允许用户在界面设置中统一选择流式或定宽内容区域。内容宽度默认流式，切换结果在当前浏览器持久化，同时保持现有
业务行为、API、权限、租户、认证和动态路由契约不变。
...
8. 提供全局内容区域宽度设置，默认流式，并允许用户切换为统一的 1280px 定宽模式。
...
| US-007 | Must | 作为界面使用者，我希望所有页面的内容区域宽度由统一设置控制，并默认使用流式布局，以便按显示器和个人偏好选择空间利用方式。 | 首页、管理页面、内容页面和工具页面 | 界面设置、主内容宿主 |
...
- 界面设置中的流式/定宽内容区域选项、默认值、本地持久化和旧设置兼容。
...
- 处理规则：
  ...
  2. 工作区型用于表格、树表、配置和批量操作；内容型用于详情、个人中心、阅读和偏产品配置页面；专项沉浸型用于代码、富文本、媒体或复杂主子编辑器。
  3. 页面类型负责页头尺寸、Section 间距和响应式密度，不再单独决定内容区域最大宽度。
  4. 全部路由主内容由全局内容宽度设置统一控制；默认流式使用可用满宽，定宽模式使用统一 1280px 最大宽度并居中。
  5. 旧本地设置没有内容宽度字段或字段值非法时必须回落为流式；恢复默认设置后也必须回到流式。
  6. 标准页面统一承担面包屑、标题、描述、状态、页级操作和内容区域，不允许每个页面复制不同标题栏。
- 输出：全部活动页面的布局分类和稳定页面骨架。

#### 5.3.2 验收标准

- `AC-009`：Given 活动业务页面清单，When 完成基础归一化，Then 每个页面都有唯一布局类型和对应公共容器或专项说明。
- `AC-010`：Given 默认流式宽度下的工作区型列表页面，When 在 1440px 和 1024px 展示，Then 表格有效利用宽度，搜索、工具栏和分页保持稳定间距。
- `AC-011`：Given 首次使用、旧本地设置或恢复默认设置，When 打开任一路由页面，Then 内容区域使用流式宽度；Given 用户选择定宽，Then 所有路由主内容统一限制为 1280px 最大宽度并居中。
...
- 处理规则：
  ...
  2. 所有页面默认使用流式内容区域；用户选择定宽后，工作区型、内容型和专项沉浸型页面统一使用 1280px 最大宽度。
  3. 内容型页面通过标题、描述、分组和纵向留白保持阅读体验，不再依赖强制定宽区分页面类型。
  4. 详情、个人中心、提示词等偏产品页面可以使用更清晰的信息分组和只读展示，不强制套用标准表格卡片。
  5. 工作区和内容型页面共享主题、字体、按钮、状态和浮层规则，不形成两套产品。
  6. `system/param.vue`、`authority/role.vue`、`asset/prompt.vue` 分别作为标准 CRUD、复杂授权和偏产品页面代表场景。
...
- `AC-036`：Given 默认流式宽度下的提示词管理代表页，When 浏览、编辑、预览和查看版本，Then 主列表有效利用可用宽度，弹层仍保持内容型产品体验且不脱离 Saber 主框架。
- `AC-037`：Given 用户切换流式或定宽，When 浏览首页、工作区型、内容型和专项沉浸型页面并刷新浏览器，Then 所有路由主内容立即采用并持久化同一宽度模式，页面类型语义和窄屏可用性不受影响。
...
| BR-011 | 内容区域宽度是用户级全局设置，默认流式；页面不得通过局部最大宽度覆盖全局选择，弹窗、抽屉、侧栏和顶栏不受该设置影响 | 主内容区域 | 移除局部覆盖并恢复统一设置 |
...
| 界面设置 | 选择所有路由页面的内容区域宽度 | 切换流式/定宽、恢复默认设置 | 已登录用户 | 现有界面设置抽屉 |
...
| 内容宽度 | 默认流式；切换定宽后全部路由主内容统一居中并限制为 1280px，刷新后保持选择 |
...
| 前端本地设置 | 是 | 在现有 `AppSetting` 中增加 `contentWidth`，沿用当前浏览器本地持久化，不新增后端用户偏好接口 | [详细设计](../design/DESIGN-REQ-2026-012-ui-normalization-spacious-visual-refresh.md) |
...
| ITEM-007 | 问题 | 固定 1280px 内容宽度导致首页和提示词管理在超宽视口无法有效利用空间 | 产品/设计/前端 | 视觉原型评审 | 已确认改为全局可选，默认流式，定宽继续使用 1280px |
...
- [ ] 最新工程门禁全部通过；类型检查、生产构建和 diff 检查通过，严格样式审计仍被既有登录页间距命中阻断。
- [x] 工作区型、内容型和专项沉浸型页面边界已登记并落实到全部活动路由页。
- [x] 白色主工作区、浅色导航、松散间距和分级圆角已实现并完成代表页面检查。
- [x] 内容宽度方案已由用户确认改为全局流式/定宽选择，默认流式；完整视口回归待执行。
...
| --- | --- | --- | --- | --- |
| 产品范围 | 内容宽度方案已确认，其余待评审 | 用户/Codex | 2026-09-16 | 所有路由主内容统一支持流式/定宽，默认流式 |
| 视觉基线 | 待评审 | 待指定 | 待指定 | 确认白色主工作区、导航层级、圆角与间距范围 |
| 前端技术 | 待评审 | 待指定 | 待指定 | 确认 Token、审计门禁、公共组件和迁移批次 |
| 测试可验收性 | 执行中 | Codex | 2026-09-16 | 宽度切换与多视口待执行；严格样式审计存在既有阻断项 |
...
| 2026-09-16 | 0.6 | 将内容区域宽度改为用户级全局设置，提供流式/定宽选项并默认流式；页面类型不再决定最大宽度，旧设置与恢复默认均回落为流式 | 用户反馈首页和提示词管理被 1280px 固定内容宽度限制，并确认由界面设置统一管理 | AppSetting、界面设置、主内容宿主、PageContainer、全部路由页面和宽度回归 | Codex |
